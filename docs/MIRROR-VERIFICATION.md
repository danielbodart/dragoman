...
| `sandbox.network.allowedDomains`/`deniedDomains` (specific hosts) | Codex network proxy + permission profile | ✅ **built + verified** (`profile.integration.test.ts` — a denied host is blocked end-to-end through ThreadRuns; the mirror now emits a permission profile carrying scope + network, written to an isolated CODEX_HOME) | **Mechanism found & verified live 2026-08-29.** Per-host filtering runs through Codex's **network proxy**, which only enforces when `features.network_proxy.enabled = true` — domain rules are *silently ignored* otherwise (that's why setting `networkAccess:true` alone let both hosts through). The rules live in a **permission profile**: `[permissions.<name>]` `extends=":workspace"`, `network.enabled=true`, `network.domains = { "host" = "allow"/"deny" }` (**deny wins**). Selected per-thread via `thread/start`'s `permissions:"<name>"` field (mutually exclusive with the `sandbox` enum). Proven: with the profile in `config.toml`, a turn got `example.com`→200 and `example.org`→*"Network access blocked: domain is explicitly denied by policy"*. **Implementation caveat:** the profile must exist in `config.toml` — the app-server `config` map does dotted *scalar* overrides and **can't inline-build the `[permissions]` table** (verified: `-32600 default_permissions requires a [permissions] table`). So implementing this means Dragoman manages a codex permission profile and switches `thread/start` from the `sandbox` enum to a `permissions` profile — the same profile route also carries the fs rules below, so it unlocks **both** deferred mappings through one engine. |
| `sandbox.filesystem.allowRead/denyRead/allowWrite/denyWrite` | profile `[permissions.<id>.filesystem]` path→access map (`allowRead`→`read`, `allowWrite`→`write`, `denyWrite`→`read` downgrade, `denyRead`→`deny`) | ✅ **built + verified** | **The third profile axis** ([`FILESYSTEM-MAPPING.md`](FILESYSTEM-MAPPING.md)). Read-deny verified model-free (`filesystem.integration.test.ts` — `denyRead` → read refused, while a sibling read and a cwd write still succeed, so the table AUGMENTS the base, not replaces it); the write-carve verified on one real turn (`denyWrite` on a subpath of a writable root → root write lands, subpath stays read-only). **Two-token gotcha locked live:** the enforced TOML key is `filesystem` (not `file_system`) and the deny value is `deny` (not `none`) — both wrong tokens load silently and enforce nothing. **This SUPERSEDES B1's "reads unrestricted"** — that held for the deprecated sandbox enum; the profile's `filesystem` `deny` blocks reads. Fold precedence per unique path (deny wins): `denyRead`→deny ▸ `denyWrite`→read ▸ `allowWrite`→write ▸ `allowRead`→read; absolute paths anchor top-level, relative/glob under `:workspace_roots`. |
