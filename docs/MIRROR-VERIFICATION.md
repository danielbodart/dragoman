...
| `sandbox.network.allowedDomains`/`deniedDomains` (specific hosts) | none reachable | ⛔ upstream-blocked | **Probed 2026-08-29, three paths, none work.** (1) Typed `SandboxPolicy` is a coarse `networkAccess` bool — no host field. (2) The freeform `config` map on `thread/start` (`experimental_network` with `domains`/`allowed_domains`/`denied_domains`) is *accepted but not enforced* — a turn allowed `example.com` and denied `example.org`, both returned HTTP 200; the coarse bool overrode it. (3) The reactive `NetworkPolicyAmendment` never fires — under `networkAccess:false`+untrusted, only a command approval fires; the sandbox hard-blocks network at the OS level (`Could not resolve host`) with no per-host ask. `NetworkRequirements`/`ConfigRequirements` are a **managed-policy READ** surface, not client-settable on `thread/start`. So per-host is coarse-only through the app-server — an **upstream** question for Codex (expose per-host network on `thread/start`, or document how `experimental_network` is driven), not a local workaround. The coarse bool (C1–C3 above) stays correct and verified. |
